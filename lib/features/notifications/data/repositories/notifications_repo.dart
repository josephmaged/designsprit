...
class NotificationsRepo extends BaseNotificationsRepo{
  ...
  @override
  Future<Either<Failure, List<ApiResponse>>> updateNotifications(UpdateNotificationsParameters parameters) async {
    try {
      final result = await baseNotificationsRemoteDataSource.updateNotifications(parameters);
      return Right(result);
    } on ServerException catch (failure) {
      return Left(ServerFailure(failure.errorMessageModel.statusMessage));
    }
