...
class ProjectStepsRepo extends BaseProjectStepsRepo {
  ...
  @override
  Future<Either<Failure, List<Installments>>> getProjectInstallments(
      GetProjectInstallmentsParameters parameters) async {
    try {
      final result = await baseProjectStepsDataSource.getInstalments(parameters);
      return Right(result);
    } on ServerException catch (failure) {
      return Left(ServerFailure(failure.errorMessageModel.statusMessage));
    }
